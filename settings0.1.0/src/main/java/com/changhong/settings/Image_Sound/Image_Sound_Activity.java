            ...
            switch (v.getId()) {
                case R.id.clarity_setting:
                    ...
                    if (keyCode == KeyEvent.KEYCODE_DPAD_RIGHT) {
                        ...
                        if (current_clarity == 0) {
                            current_clarity = 1;
                            Settings.Secure.putInt(mContext.getContentResolver(), "default_player_quality",1);
                            clarity_setting_switch.setText(getResources().getText(R.string.setting_quality_smooth));
                        } else if (current_clarity == 1) {
                            current_clarity = 2;
                            Settings.Secure.putInt(mContext.getContentResolver(), "default_player_quality",2);
                            clarity_setting_switch.setText(getResources().getText(R.string.setting_quality_auto));
                        } else if (current_clarity == 2) {
                            current_clarity = 0;
                            Settings.Secure.putInt(mContext.getContentResolver(), "default_player_quality",0);
                            clarity_setting_switch.setText(getResources().getText(R.string.setting_quality_clarity));
                        }
                ...
                case R.id.image_ratio:
                    ...
                    if (keyCode == KeyEvent.KEYCODE_DPAD_LEFT || keyCode == KeyEvent.KEYCODE_DPAD_RIGHT) {
                        ...
                        if (current_ratio == 0) {
                            current_ratio = 1;
                            Settings.Secure.putInt(mContext.getContentResolver(), "default_screen_ratio",1);
                            image_ratio_switch.setText(getResources().getText(R.string.setting_ratio_full));
                        } else {
                            current_ratio = 0;
                            Settings.Secure.putInt(mContext.getContentResolver(), "default_screen_ratio",0);
