...
public class Settings extends Activity implements View.OnClickListener {
    ...
    private boolean isSecurityOpen() {
        try {
            int flag = android.provider.Settings.Secure.getInt(mContext.getContentResolver(), "password_protect_state");
            if (flag == 1) {
                return true;
            } else return false;
    ...
    private void showDialogSecurityPasswordProtection() {
        dialog = new SecurityPasswordDialog(mContext, android.R.style.Theme_Translucent_NoTitleBar_Fullscreen,
            ...
            public void CorrectCallBack() {
                Toast.makeText(mContext,"密码输入正确," +
                        "系统安全已开启...",Toast.LENGTH_LONG).show();
            }

            @Override
            public void InCorrectCallBack() {
                Toast.makeText(mContext,"密码输入错误," +
                        "请重试",Toast.LENGTH_LONG).show();
            }

            @Override
            public void EmptyCallBack() {
                Toast.makeText(mContext,"密码不能为空，请重试",
