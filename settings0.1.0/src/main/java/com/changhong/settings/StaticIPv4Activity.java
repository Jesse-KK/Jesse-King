...
//    private String stack;
    ...
    private NetworkV4V6Utils.NetInfo mNetInfoV4 = null;
    ...
    private final BroadcastReceiver mEthReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (IpConfiguration.IpAssignment.STATIC != mIpConfigV4.ipAssignment) {
                Log.i(TAG, "ipv4 mode is not static, now mode: " + mIpConfigV4.ipAssignment);
                return;
            }
...
//        Log.i(TAG, "onClick: isEnableIpv4=" + isEnableIpv4 + ",isEnableIpv6=" + isEnableIpv6 + ",stack=" + stack);
    ...
    private void connectStatic() {
        if (validateIpConfigFields_yao() != 0) {
            startStaticV4_yao();
        }
//        if (validateIpConfigFields_cao(mLinkProperties)==0){
//            startStaticV4_cao();
//        }
    }

    private void startStaticV4_yao() {
        Log.e(TAG, "startStaticV4_yao: 开始连接：：" );
        InetAddress ip = NetworkUtils.numericToInetAddress(mNetInfoV4.ip);
        InetAddress gateway = NetworkUtils.numericToInetAddress(mNetInfoV4.gateway);
        InetAddress dns1 = NetworkUtils.numericToInetAddress(mNetInfoV4.dns1);
        InetAddress dns2 = NetworkUtils.numericToInetAddress(mNetInfoV4.dns2);
        Log.i(TAG, "---startStaticV4 , dns1:" + dns1 + " dns2:" + dns2);
//        DhcpInfo dhcpInfo = new DhcpInfo();
//        Log.e(TAG, "startStaticV4_yao: 开始连接静态IPv4");
...
//        dhcpInfo.dns2 = NetworkUtils.inetAddressToInt(idns2);
        ...
        IpConfiguration ipv4Config = new IpConfiguration();
        ipv4Config.ipAssignment = IpConfiguration.IpAssignment.STATIC;
        ipv4Config.staticIpConfiguration = new StaticIpConfiguration();

        ipv4Config.staticIpConfiguration.ipAddress = new LinkAddress(ip, prefixV4);
        ipv4Config.staticIpConfiguration.gateway = gateway;
        //此处添加dns需先添加dns2,再添加dns1，否则获取的时候会先获取到dns2
        ipv4Config.staticIpConfiguration.dnsServers.add(dns2);
        ipv4Config.staticIpConfiguration.dnsServers.add(dns1);

        mEthManager.setIpv4Configuration(ipv4Config);
    ...
    //    private void startStaticV4_cao() {
//        mEthManager.enableIpv4(true);
//        mEthManager.disconnectIpv4();
...
//        mEthManager.connectIpv4();
//    }
    private int validateIpConfigFields_yao() {
        ...
        mNetInfoV4 = getNetInfo();
        mNetInfoV4.ip = ip;
        mNetInfoV4.mask = mask;
        mNetInfoV4.gateway = gateway;
        mNetInfoV4.dns1 = dns;
        mNetInfoV4.dns2 = dns2;
        Log.e(TAG, "validateIpConfigFields_yao: 备用dns的地址"+mNetInfoV4.dns2);
        if (mNetInfoV4.dns2.isEmpty()) {
            mNetInfoV4.dns2 = "0.0.0.0";
            Log.e(TAG, "validateIpConfigFields_yao:地址为空： "+mNetInfoV4.dns2 );
        }
        if(mNetInfoV4.dns2.equals("...")){
            mNetInfoV4.dns2 = "0.0.0.0";
            Log.e(TAG, "validateIpConfigFields_yao:格式正确的dns： "+ mNetInfoV4.dns2);
        }
        result = NetworkV4V6Utils.validateIpv4ConfigFields(mNetInfoV4);
        switch (result){
            case IP_ERR:
    ...
    private int validateIpConfigFields_cao(LinkProperties linkProperties) {
        Log.i(TAG, "!!-------validateIpConfigFields-----!!");
